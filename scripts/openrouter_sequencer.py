...
Manager (Grok) owns briefs and integration. Workers are chat completions only.

Env:
  RUN_DIR             run directory (tasks.json, briefs/, workspace/)
...
def worker_generate(prompt: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "HTTP-Referer": "https://github.com/MrJ55/grok-zero-anneal",
            "X-Title": "grok-zero-anneal-sequencer",
        },
        method="POST",
    )
...
def main() -> int:
    ...
    if not tasks_path.exists():
        raise SystemExit(f"missing {tasks_path}; set RUN_DIR to a run directory")
    ...
    for task in tasks:
        ...
        while attempts < max_attempts:
            ...
            out_path = out_dir / f"{tid}-a{attempts}.md"
